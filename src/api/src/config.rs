...
///
/// The VMM compiles source inside a guest, so accepting arbitrarily large
...
pub struct ApiConfig {
    ...
    /// Optional bearer token used by the API when calling the VMM broker.
    pub vmm_auth_token: Option<String>,
...
impl fmt::Debug for ApiConfig {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("vmm_endpoint", &self.vmm_endpoint)
            .field(
                "auth_token",
            ...
            .field(
                "vmm_auth_token",
                &self.vmm_auth_token.as_ref().map(|_| "[redacted]"),
            )
...
impl ApiConfig {
    ...
    pub fn try_from_env() -> Result<Self, ConfigError> {
        ...
        let vmm_auth_token = optional_token("CLOUDLET_VMM_AUTH_TOKEN")?;
        ...
        Ok(Self {
            bind_host,
            bind_port,
            vmm_endpoint: env::var("CLOUDLET_VMM_ENDPOINT")
