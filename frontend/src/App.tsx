  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><Button variant="ghost" className="icon-button menu-button" aria-label="Open navigation" onClick={() => setSidebar(true)}><Menu size={20} /></Button><span>Workspace</span><span className="slash">/</span><strong>{page}</strong></div><div className="topbar-right"><span className="local-chip"><span className="dot" />Local</span><Button variant="ghost" size="icon-sm" className="icon-button" aria-label="Refresh connection" title="Refresh connection" onClick={() => setRefreshKey(key => key + 1)} disabled={refreshing}><RefreshCw size={16} className={refreshing ? 'spin' : ''} /></Button><Button variant="ghost" size="icon-sm" className="icon-button" aria-label="Connection help" onClick={() => navigate('Settings')}><CircleHelp size={18} /></Button></div></header>
      <main id="main-content">
        ...
        {page === 'Models' && <div className="details-grid"><Card className="panel info-panel gap-0 p-6"><div className="info-title"><Zap size={22} /><h2>Model connectivity</h2><UiBadge variant="outline">NOT CONFIGURED</UiBadge></div><p>BoxLite supplies sandbox execution, not model inference. The legacy llmman bridge is not attached to this runtime.</p><div className="info-callout"><ShieldCheck size={19} /><span>Guest networking is disabled. Model clients cannot reach local or hosted providers from these templates.</span></div><p>Model integration needs an explicit outbound policy and secret handling before guest access is enabled.</p><a className="text-button" href="https://github.com/llmmanorg/llmman" target="_blank" rel="noreferrer">llmman documentation<ExternalLink size={14} /></a></Card><Card className="panel info-panel gap-0 p-6"><h2>Current boundary</h2><ul className="limits"><li>No provider keys stored in this browser</li><li>No model downloads or GPU setup performed</li><li>Existing legacy bridge code retained separately</li></ul></Card></div>}
